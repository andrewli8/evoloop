...
3. **Evidence.** The scan only sees `TODO`s, fix commits, open GitHub issues, pain words in docs, and git churn/revert hotspots (`evidence.churn_enabled`, on by default). Real feedback beats all of that: drop support tickets, user quotes, or analytics notes into `.evoloop/evidence/*.md`, one item per line.
